...
public class AccountServiceExposure {
    ...
    public Response list(@Context UriInfo uriInfo, @Context Request request) {
        List<Poll> polls = archivist.listAccounts();
        return new EntityResponseBuilder<>(polls, list -> new AccountsRepresentation(list, uriInfo))
            .maxAge(10)
            .build(request);
    }
    ...
    @Produces({ "application/hal+json" })
    @LogDuration(limit = 100)
    @ApiOperation(value = "Get single account", nickname = "getPoll")
    @ApiResponses(value = {
    ...
    public Response get(@PathParam("regNo") @Pattern(regexp = "^[0-9]{4}$") String regNo,
        ...
        return new EntityResponseBuilder<>(poll, acc -> new AccountRepresentation(acc, acc.getVotes(), uriInfo))
            .name(CONCEPT_NAME)
    ...
    public Response createOrUpdate(@PathParam("regNo") @Pattern(regexp = "^[0-9]{4}$") String regNo,
        ...
        Optional<Poll> acc = archivist.findAccount(regNo, accountNo);
        Poll a;
        if (acc.isPresent()) {
            a = acc.get();
            a.setName(account.getName());
        } else {
            a = new Poll(regNo, accountNo, account.getName());
        }
        archivist.save(a);
